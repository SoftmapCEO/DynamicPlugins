#!/usr/bin/env python3
"""
Kimi Code CLI PreToolUse hook -- 插件热注入

在工具调用前分析上下文，匹配禁用 plugin 的 skill，
通过 stdout 输出注入 skill 摘要。

输入: stdin JSON (Kimi hook 上下文)
输出: stdout JSON {"additionalContext": "..."}

与 Claude/Codex/Gemini 版本共享相同的反向索引格式和匹配算法。
"""
...
log = get_logger("kimi_prompt_inject")

CACHE_FILE = Path(__file__).resolve().parent.parent / "cache" / "skills_index.json"
SIGNAL_FILE = Path(tempfile.gettempdir()) / "kimi-plugin-hotload-signal.json"
MAX_SKILLS = 2
MIN_MATCH_SCORE = 3
...
def main():
    ...
    if SIGNAL_FILE.exists():
        try:
            ...
            result = {"additionalContext": f"[Kimi 插件管理] {signal.get('action', '')}: {signal.get('details', '')}"}
