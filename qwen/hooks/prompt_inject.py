#!/usr/bin/env python3
"""
Qwen Code CLI UserPromptSubmit hook -- 插件热注入

在用户提交消息时，匹配禁用 extension 的 skill，
通过 hookSpecificOutput.additionalContext 注入摘要。

Qwen hook 输入 (stdin): {"prompt": "...", "session_id": "...", "cwd": "..."}
...
log = get_logger("qwen_prompt_inject")

CACHE_FILE = Path(__file__).resolve().parent.parent / "cache" / "skills_index.json"
SIGNAL_FILE = Path(tempfile.gettempdir()) / "qwen-plugin-hotload-signal.json"
MAX_SKILLS = 2
MIN_MATCH_SCORE = 3
...
def main():
    ...
    if SIGNAL_FILE.exists():
        try:
            ...
            out = {
                "hookSpecificOutput": {
                    "additionalContext": f"[Qwen Extension 管理] {signal.get('action', '')}: {signal.get('details', '')}",
                }
            }
