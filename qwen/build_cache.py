#!/usr/bin/env python3
"""
Qwen Code CLI skills 索引缓存构建器

扫描 Qwen extensions 中的 skills，构建反向索引用于关键词匹配。
...
log = get_logger("qwen_build_cache")
...
def get_disabled_extensions() -> set:
    """读取 settings.json 中的 disabledExtensions。"""
...
def build_index():
    """构建 Qwen extensions 的反向索引。"""
    log.info("开始构建 Qwen extensions 缓存")
...
def main():
    ...
    print(f"Qwen extensions 缓存已构建: {OUTPUT}")
