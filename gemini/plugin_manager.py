...
根据项目内容自动启用/禁用 Gemini extensions。
扫描项目目录中的文件扩展名、配置文件和包依赖来决定需要哪些扩展。

Gemini extensions 位于: ~/.gemini/extensions/
...
log = get_logger("gemini_plugin_manager")
...
def determine_needed_extensions(project_dir: str, extensions: dict) -> set:
    ...
    for rule in rules.get("conditional", []):
        plugin_raw = rule["plugin"]
        plugin = plugin_raw.lower()
        if rule.get("_manual"):
            continue
...
def apply_extensions(needed: set, extensions: dict, dry_run: bool = False) -> dict:
    """更新 settings.json 中的 extensions 配置。"""
    settings = load_settings()

    # Gemini 的 extensions 禁用机制: disabledExtensions 数组
...
def main():
    ...
    if not dry_run:
        ...
        ensure_plugins_available("gemini", needed, set(extensions.keys()))
        extensions = discover_extensions()  # 重新发现（可能有新安装）

    changes = apply_extensions(needed, extensions, dry_run=dry_run)

    prefix = "[预览] " if dry_run else ""
    print(f"\n{prefix}Gemini Extensions 配置")
    print("=" * 50)
    if changes["enabled"]:
