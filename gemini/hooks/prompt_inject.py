#!/usr/bin/env python3
"""
Gemini CLI BeforeAgent hook -- 插件热注入

在 Agent 处理前分析用户消息，匹配禁用 extension 的 skill，
通过 hookSpecificOutput.additionalContext 将摘要注入。

Gemini hook 输入: stdin JSON (包含用户消息)
Gemini hook 输出: stdout JSON
  {"hookSpecificOutput": {"hookEventName": "BeforeAgent", "additionalContext": "..."}}

与 Claude/Codex 版本共享相同的反向索引格式和匹配算法。
"""
...
log = get_logger("gemini_prompt_inject")

CACHE_FILE = Path(__file__).parent.parent / "cache" / "skills_index.json"
SIGNAL_FILE = Path(tempfile.gettempdir()) / "gemini-plugin-hotload-signal.json"
MAX_SKILLS = 2
MIN_MATCH_SCORE = 3
...
def emit_gemini_output(additional_context: str):
    """输出 Gemini BeforeAgent hook 格式的 JSON。"""
    result = {
        "hookSpecificOutput": {
...
def main():
    ...
    if SIGNAL_FILE.exists():
        try:
            ...
            emit_gemini_output(
                f"[Gemini Extension 管理] {signal.get('action', '')}: {signal.get('details', '')}"
            )
            return
    ...
    if parts:
        emit_gemini_output(
            "[Skill hint] The following disabled extension skills match your query. "
